...
### 11.7 Voice Selection (New — Per-Book, Not Just Global)

Each book gets its own voice choice, set at import time and changeable later —
not a single app-wide default everyone's books inherit.

- **English (Kokoro):** the full ~54-voice set is downloaded (each voice file
  is tiny, ~500KB, so there's no reason to restrict at download time). The
  import screen (and a book's settings) shows a voice picker — name + a short
  sample-preview play button — and the user's choice is stored per book.
- **Hindi (Indic Parler-TTS):** voice isn't a discrete file — it's set via a
  text description prompt at generation time. Ship 2–3 default presets (e.g.
  a clear neutral voice, a warmer/slower one) the user picks from in the same
  per-book voice picker; store the selected preset (or custom prompt string,
  if allowed later) per book.
- **Changing voice after narration has started:** picking a different voice
  for a book that's already partially narrated should prompt the user
  ("This will re-narrate already-generated chapters in the new voice —
  continue?") rather than silently mixing voices across chapters within one
  book.
- **Data model addition:**
  ```sql
  books(
    ...,
    voice_id           -- Kokoro voice name (e.g. 'af_sarah') for English books
    voice_prompt        -- Hindi preset id or description string for Hindi books
  )
  ```
  `chunks.tts_engine` (already in the schema) stays as-is; `voice_id`/
  `voice_prompt` on `books` is what the engine call reads at generation time.
